...
```bash
sam-sel-py39-test$ sam logs -n HelloWorldFunction --stack-name sam-sel-py39-test --tail
```
...
```bash
sam-sel-py39-test$ pip install pytest pytest-mock --user
sam-sel-py39-test$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam-sel-py39-test
```
